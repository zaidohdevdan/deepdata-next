...
export default async function NoturnaEscalasPage() {
  const globalConfig = await getConfigValues()
  const session = await auth()
  
  let initialPoliciaisFixos = []
  try {
    initialPoliciaisFixos = JSON.parse(globalConfig.escalaPoliciaisFixos_noturna || "[]")
  } catch (error) {
    console.error("Error parsing fixed police officers for noturna:", error)
  }
  ...
  return (
    <EscalasContainer 
      ...
      initialPostosConfig={globalConfig.escalaPostosConfig_noturna}
      initialHoraInicio={globalConfig.escalaHoraInicio_noturna}
      initialHoraFim={globalConfig.escalaHoraFim_noturna}
      initialNumFaixas={globalConfig.escalaNumFaixas_noturna}
    />
  )
}
