...
export default async function JantaEscalasPage() {
  const globalConfig = await getConfigValues()
  const session = await auth()
  
  let initialPoliciaisFixos = []
  try {
    initialPoliciaisFixos = JSON.parse(globalConfig.escalaPoliciaisFixos_janta || "[]")
  } catch (error) {
    console.error("Error parsing fixed police officers for janta:", error)
  }
  ...
  return (
    <EscalasContainer 
      ...
      initialPostosConfig={globalConfig.escalaPostosConfig_janta}
      initialHoraInicio={globalConfig.escalaHoraInicio_janta}
      initialHoraFim={globalConfig.escalaHoraFim_janta}
      initialNumFaixas={globalConfig.escalaNumFaixas_janta}
    />
  )
}
